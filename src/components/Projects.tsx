...
function Projects({ section, toggleProjectsModal }: ProjectsProp) {
  ...
  const projList = recentProjects.map((project) => {
    return (
      <article
        className="flex flex-col justify-center items-start p-1.5 md:p-2 space-y-1 border-light-border/20 dark:border-dark-border/20 border-1 rounded-md transform hover:-translate-y-1 transition duration-300 ease-in-out"
        key={project.id}
      >
