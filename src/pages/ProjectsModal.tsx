...
function ProjectsModal({ theme, copyrightNote }: ProjectsModalProp) {
  ...
  const projectCard = allProjects.map((project) => {
    return (
      <article
        className="card w-full md:min-h-[480px] flex flex-col justify-center items-center border-2 bg-light-bg border-light-border/15 dark:bg-dark-card dark:border-dark-border/15 animate-fade-in"
        key={project.id}
      >
        ...
        <div className="w-full flex flex-col justify-center items-start p-4 space-y-3 grow">
          ...
          <div className="w-full flex flex-wrap justify-start items-center gap-1.5">
            {project.techstack.map((language, i) => {
              return (
                <div
                  className="badge badge-sm lg:badge-md text-light-font dark:text-dark-font/80 bg-light-button dark:bg-dark-button rounded-full border-none"
                  key={i}
                >
                  {language}
                </div>
          ...
          <a
            className="w-20 h-10 flex justify-center items-center rounded-md cursor-pointer bg-light-button dark:bg-dark-button hover:bg-dark-button/15 dark:hover:bg-light-button/10"
            href={project.link}
            target={"_blank"}
          >
  ...
  return (
    <section
      className={`${
        theme ? "dark" : ""
      } flex min-h-screen bg-light-bg dark:bg-dark-bg`}
    >
      <div className="max-w-full h-full flex flex-col justify-between items-center p-6 md:px-10 xl:px-20 font-geist text-light-font dark:text-dark-font/80 overflow-auto">
